      ...
      <div class="snoop-forum-detail">
        ...
        <div class="snoop-forum-comments-section">
          ...
          >
            ...
            <div v-if="comment.replies?.length" class="snoop-forum-nested-replies">
              ...
              >
                ...
                <div class="snoop-forum-comment-body">
                  <div class="snoop-forum-comment-user">
                    {{ reply.username }}
                    <span v-if="reply.replyTo" class="snoop-forum-reply-indicator">回复 {{ reply.replyTo }}</span>
            ...
            <div v-if="replyTarget?.comment === comment" class="snoop-forum-nested-input-wrap">
              <input
                v-model="replyInput"
                class="snoop-forum-nested-input"
                :placeholder="'回复 ' + (replyTarget?.targetName || comment.username) + '...'"
                maxlength="200"
                @keydown.enter="submitReply(selectedPost, comment)"
                @blur="replyTarget = null; replyInput = ''"
                ref="replyInputRef"
              />
              <button
...
/** Start replying to a specific person in the comment thread (楼中楼) */
function startReply(comment, targetName) {
  replyTarget.value = { comment, targetName }
  replyInput.value = ''
  nextTick(() => {
...
async function submitReply(post, comment) {
  ...
  // Determine who this user is replying to
  const replyToName = replyTarget.value?.targetName || comment.username
  ...
  // If user replied directly to the phone owner → phone owner replies back directly
  if (replyToName === ownerName) {
    await ownerNestedDirectReply(post, comment, userNested)
    return
  }

  // Otherwise: the person user replied to replies back + phone owner chimes in as observer
  if (replyToName !== '匿名网友') {
  ...
  // Phone owner chimes in from a third-party perspective
...
/** Phone owner directly replies to the user who replied to them */
async function ownerNestedDirectReply(post, targetComment, userNested) {
  const ownerName = props.contact?.name || '楼主'
  replyingTo.value = ownerName + ' 正在回复...'
  try {
    const ownerReply = await generateOwnerDirectNestedReply(props.contact, post, targetComment, userNested)
    setTimeout(() => {
      if (!targetComment.replies) targetComment.replies = []
      targetComment.replies.push(ownerReply)
      replyingTo.value = ''
...
.snoop-forum-reply-indicator {
  font-size: 10px;
  color: #8e8e93;
  font-weight: 400;
}
.snoop-forum-reply-indicator::before {
  content: ' ';
}
