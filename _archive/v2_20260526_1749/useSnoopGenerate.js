...
export async function generateOwnerReplyToComment(contact, post, userComment) {
  ...
  const name = contact.name || '楼主'

  const systemParts = [baseSystemPrompt]
  systemParts.push('你正在逛论坛，看到一篇帖子。')
  systemParts.push('你在帖子评论区看到一条新的评论，你想回复这条评论。')
  systemParts.push(`请以「${name}」的身份回复，语气自然生活化，像真实的论坛用户在评论。`)
  systemParts.push('只说一句话作为回复，不要带任何前缀或格式，不要加引号，就像真的在论坛评论区打字那样。回复不要太长，15-40字左右。')

  const systemPrompt = systemParts.join('\n\n')

  const userPrompt = `帖子《${post.title || ''}》：${post.content}\n\n你在评论区看到一条新评论：\n「${userComment.username}」说：${userComment.content}\n\n请回复${userComment.username}：`
...
/**
 * Phone owner directly replies to the user in a nested reply thread (not as observer)
 * Used when user replies to the phone owner's existing reply
 */
export async function generateOwnerDirectNestedReply(contact, post, targetComment, userReply) {
  const cfg = getResolvedSnoopConfig(contact)
  if (!cfg?.url || !cfg?.key) throw new Error('未配置 API：请先在角色配置里填写接口地址和密钥')
  ...
  const systemParts = [baseSystemPrompt]
  systemParts.push('你正在逛论坛，看到一篇帖子。')
  systemParts.push('你在评论区跟人聊着天，有人回复了你，你想回复回去。')
  systemParts.push(`请以「${name}」的身份直接回复，语气自然生活化，就像在论坛评论区跟人聊天一样。`)
  systemParts.push('只说一句话作为回复，不要带任何前缀或格式，不要加引号。回复不要太长，15-40字左右。')

  const systemPrompt = systemParts.join('\n\n')

  const userPrompt = `帖子《${post.title || ''}》：${post.content}\n\n你之前说：${targetComment.content}\n\n匿名网友回复了你：${userReply.content}\n\n请回复匿名网友：`
...
export async function generateOwnerNestedReply(contact, post, targetComment, userReply) {
  ...
  // 用户的回复如果不在已有回复列表里才追加（防止重复）
  const userAlreadyIn = targetComment.replies?.some(
    r => r.username === userReply.username && r.content === userReply.content
  )
  if (!userAlreadyIn) {
  ...
  const systemParts = [baseSystemPrompt]
  systemParts.push('你正在逛论坛，看到一篇帖子。')
  systemParts.push('你在评论区看到一段别人正在进行的讨论，你作为旁观者想插一句话。')
  systemParts.push(`请以「${name}」的身份回复，语气自然生活化，像真实的论坛用户随手搭句话。`)
  systemParts.push('只说一句话作为回复，不要带任何前缀或格式，不要加引号。回复不要太长，15-40字左右。')
 ...
 */
export async function generateBystanderReply(contact, post, targetComment, userReplyText) {
  const cfg = getResolvedSnoopConfig(contact)
  if (!cfg?.url || !cfg?.key) throw new Error('未配置 API：请先在角色配置里填写接口地址和密钥')
  ...
  const systemParts = [baseSystemPrompt]
  systemParts.push('你正在逛论坛，看到一篇帖子。')
  systemParts.push(`你在评论区看到有人回复了你，你想回复回去。`)
  systemParts.push(`请以「${targetComment.username}」的身份回复，语气自然生活化，不要复读自己的话。`)
  systemParts.push('只说一句话作为回复，不要带任何前缀或格式，不要加引号。回复不要太长，15-40字左右。')

  const systemPrompt = systemParts.join('\n\n')

  const userPrompt = `帖子《${post.title || ''}》：${post.content}\n\n你之前说：${targetComment.content}\n\n匿名网友回复了你：${userReplyText}\n\n请回复匿名网友：`
