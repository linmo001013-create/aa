<template>
  ...
  >
    <div v-if="!bootstrapStore.isHydrating" class="route-view">
...
watch(() => route.path, (to, from) => {
  ...
  const order = ['/', '/favorites', '/messages', '/groups', '/chat', '/phone', '/snoop', '/persona', '/settings', '/lorebook', '/moments', '/theme', '/album', '/vn', '/meet', '/planner', '/tavern']
...
function syncActiveChatByRouteParam() {
  ...
  // 先从联系人里找
  let contact = contactsStore.contacts.find(c => String(c.id) === contactId) || null
  // 没找到再尝试从群聊数据里找
  if (!contact) {
    const group = groupsStore.getGroupById(contactId)
    if (group) {
      contact = group
    }
  }
