...
const editingGroup = computed(() => {
  ...
  // 先查 contactsStore，再查 groupsStore（有的群只存在 groupsStore 里）
  let contact = contactsStore.contacts.find(item => String(item?.id || '') === contactId) || null
  if (!contact || contact.type !== 'group') {
    contact = groupsStore.groups.find(item => String(item?.id || '') === contactId) || null
  }
...
function saveGroup() {
  ...
  if (props.isEdit && editingGroup.value) {
    // 从 contactsStore 或 groupsStore 同步更新群聊
    const contactIndex = contactsStore.contacts.findIndex(x => x.id === editingGroup.value.id)
    const groupIndex = groupsStore.groups.findIndex(x => x.id === editingGroup.value.id)
    ...
    if (contactIndex !== -1) {
      Object.assign(contactsStore.contacts[contactIndex], updatedFields)
    ...
    if (groupIndex !== -1) {
      Object.assign(groupsStore.groups[groupIndex], updatedFields)
      if (contactsStore.activeChat?.id === editingGroup.value.id && contactIndex === -1) {
        contactsStore.activeChat = groupsStore.groups[groupIndex]
      }
    }
    showToast('已更新')
  } else {
    ...
    groupsStore.groups.unshift({ ...newGroup })
    showToast('已创建')
  }
...
async function clearChatHistory() {
  ...
  if (groupIndex !== -1) {
    groupsStore.groups[groupIndex].msgs = []
    if (contactsStore.activeChat?.id === editingGroup.value.id && contactIndex === -1) {
      contactsStore.activeChat = groupsStore.groups[groupIndex]
    }
  }
...
async function deleteGroup() {
  ...
  groupsStore.groups = groupsStore.groups.filter(g => g.id !== deletedId)
