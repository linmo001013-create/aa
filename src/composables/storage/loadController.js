 ...
 * @param {() => Promise<unknown>} [options.getFileBackup]
...
export async function loadStoredSnapshot(options) {
  ...
  // 文件系统备份兜底（Capacitor Filesystem，Android 原生存储）
  // 注意: 必须同时检查 !data（IndexedDB 返回 null）和 data 是否真的有内容
  // 因为 IndexedDB 可能返回一个空快照对象（{}）而不是 null，此时 !data 为 false，
  // 文件备份会被错误地跳过！
  const dataHasContent = data && hasUserData(prepareSnapshot(data))
  if ((!data || !dataHasContent) && typeof getFileBackup === 'function') {
    try {
      const fileBackup = await getFileBackup()
      if (fileBackup && hasUserData(fileBackup)) {
        data = fileBackup
        console.info('Recovered data from file system backup')
        showToast('已从文件备份恢复数据', 3200)
        if (!isPartialLocalBackup(fileBackup)) {
          await persistRecoveredSnapshot(data, {
            keyAppData,
            idbSet,
            normalizeLoadedAppData,
      ...
      // file backup fallback failed, continue
    }
  }
