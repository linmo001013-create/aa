  ...
  store.contacts.forEach((contact) => {
    ...
    if (Array.isArray(contact.members)) {
      contact.members = contact.members.map((member) => {
        ...
        // 备份头像数据，避免外部化/GC机制导致头像丢失
        if (member.avatarType === 'image' && typeof member.avatar === 'string' && member.avatar.startsWith('data:')) {
...
// 同时也处理 groups store 中的成员头像备份
// 因为群聊 ChatHeader 从 groupsStore.groups 获取数据，而不是从 contacts
function normalizeGroupMemberAvatarBackups(store) {
...
export const STORAGE_SNAPSHOT_MODULES = [
  {
    ...
    snapshot(context) {
      ...
      return {
        groups: externalized.groups || store.groups || []
      }
    }
  },
  {
...
export function finalizeAppliedSnapshot(deps) {
  ...
  normalizeGroupMemberAvatarBackups(deps.store)
