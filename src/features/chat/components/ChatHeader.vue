  ...
  >
    <div class="chat-header-inner flex items-center justify-between w-full h-full relative pt-app pb-2 px-2">
      ...
      <div class="chat-header-center flex-1 flex flex-col items-center justify-center cursor-pointer z-10" @click="onCenterClick">
        ...
        >
          <template v-if="contact?.members && contact.members.length > 0">
            ...
            >
              <img v-if="getMemberAvatar(member)?.type === 'image'" :src="getMemberAvatar(member)?.src" class="w-full h-full object-cover" @error="member.avatarType = 'emoji'">
              <span v-else class="text-[10px]">{{ getMemberAvatar(member)?.src || '👤' }}</span>
            </div>
          </template>
      ...
      <div class="chat-header-actions flex items-center gap-4 z-20 pr-1">
        ...
        <button v-if="isGroup" class="chat-header-action w-8 h-8 rounded-full flex items-center justify-center active:bg-black/5 dark:active:bg-white/10 transition-colors group/refresh" @click="$emit('refreshAvatars')" title="刷新群成员头像">
          <i class="ph-bold ph-arrow-clockwise text-[20px] group-active/refresh:rotate-45 transition-transform duration-200"></i>
        </button>
...
const emit = defineEmits(['back', 'editContact', 'toggleNarrations', 'startCall', 'refreshAvatars'])

const store = useSettingsStore()

// 判断头像是否为 blob URL 或 media ref（这些在刷新后是无效的）
function isInvalidMediaUrl(val) {
  if (!val || typeof val !== 'string') return true
  return val.startsWith('blob:') || val.startsWith('_media_') || val.startsWith('_member_avatar_')
}

// 直接从群成员自身获取头像，不依赖联系人 store
function getMemberAvatar(member) {
  if (!member) return { type: 'emoji', src: '👤' }

  // 从 avatarBackup 恢复（外部化机制不会碰这个字段）
  const avatar = !isInvalidMediaUrl(member.avatar) ? member.avatar : member.avatarBackup
  const avatarType = member.avatarType || 'emoji'

  if (avatarType === 'image' && avatar && !isInvalidMediaUrl(avatar)) {
    return { type: 'image', src: avatar }
  }

  if (avatar && !isInvalidMediaUrl(avatar)) {
