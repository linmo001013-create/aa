<template>
  <div class="chat-view absolute inset-0 z-30 bg-[var(--chat-bg)] flex flex-col overflow-hidden">
    ...
    <ChatHeader
      ...
      @refresh-avatars="handleRefreshAvatars"
    />
...
function handleRefreshAvatars() {
  const group = store.activeChat
  if (!group?.members || !Array.isArray(group.members)) return
  let updated = 0
  group.members.forEach(member => {
    // 成员的真实联系人ID存在 contactId 字段
    const lookupId = member.contactId || member.id
    if (!lookupId) return
    // 在联系人列表中查找匹配的联系人
    const contact = store.contacts.find(c => String(c.id) === String(lookupId))
    if (!contact?.avatar) return
    // 强制将联系人APP里的头像覆盖到群成员上
    member.avatar = contact.avatar
    member.avatarType = contact.avatarType || 'image'
    member.avatarBackup = contact.avatar
    updated++
  })
  if (updated > 0) {
    scheduleSave({ notify: false })
    showToast(`已强制刷新 ${updated} 个群成员头像`)
  } else {
    showToast(`未找到匹配的联系人头像（共 ${group.members.length} 位成员）`)
  }
}
