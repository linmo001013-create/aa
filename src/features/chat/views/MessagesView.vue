          ...
          <div v-else class="relative w-[50px] h-[50px] shrink-0">
            <div class="w-full h-full rounded-full bg-[#E9E9EB] dark:bg-gray-700 flex items-center justify-center overflow-hidden relative">
              <template v-if="item.contact.members && item.contact.members.length > 0">
                ...
                >
                  <img v-if="member.avatarType === 'image' && !failedMemberAvatars.has(item.contact.id + '_' + idx)" :src="member.avatar" loading="lazy" decoding="async" class="w-full h-full object-cover" @error="handleMemberAvatarError(item.contact.id, idx, member)">
...
function handleMemberAvatarError(contactId, idx, member) {
  const key = contactId + '_' + idx
