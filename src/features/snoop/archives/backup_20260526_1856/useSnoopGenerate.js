 ...
 */
export async function generateBystanderReply(contact, post, targetComment, userReply, triggerUsername) {
  const cfg = getResolvedSnoopConfig(contact)
  if (!cfg?.url || !cfg?.key) throw new Error('未配置 API：请先在角色配置里填写接口地址和密钥')
...
现在「${who}」回复了「${targetComment.username}」说：「${userReply}」
请以「${targetComment.username}」的身份，自然地回复「${who}」的这条回复。不要自己重复自己的话。`)

  const systemPrompt = systemParts.join('\n\n')

  const userPrompt = `帖子：${post.content}
${targetComment.username} 之前说：${targetComment.content}
${who} 回复 ${targetComment.username}：${userReply}

请以 ${targetComment.username} 的身份回复 ${who}，JSON输出：
 ...
 * @returns {Object} {username, content, time, likes}
 */
export async function generateUserReply(contact, post, userName, triggerContent, triggerUserName) {
  const cfg = getResolvedSnoopConfig(contact)
  if (!cfg?.url || !cfg?.key) throw new Error('未配置 API')
...
现在你看到了这条回复。请自然地回复${triggerUserName}，表达你的想法或反应。就像正常对话一样。`)
...
请以 ${userName} 的身份回复${triggerUserName}，JSON输出：
{"content":"回复内容","time":"刚刚","likes":数字}`
