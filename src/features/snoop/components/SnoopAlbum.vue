<template>
  <div class="snoop-tab-content">
    ...
    <!-- Content -->
    <template v-else>
      <div class="snoop-album-grid" :class="{ 'snoop-album-grid--selecting': selectable }">
        <div
          v-for="(item, idx) in items"
          :key="idx"
          class="snoop-album-cell"
          :class="{ 'snoop-album-cell--selecting': selectable, 'snoop-album-cell--selected': selectedIndices.includes(idx) }"
          @click="selectable ? $emit('toggle-select', idx) : (selectedIdx = selectedIdx === idx ? null : idx)"
        >
          <div v-if="selectable" class="snoop-album-checkbox">
...
.snoop-album-cell--selecting { cursor: pointer; }
.snoop-album-cell--selected {
  background: rgba(0, 122, 255, 0.08);
  outline: 2px solid var(--primary-color, #007aff);
  outline-offset: -2px;
}

.snoop-album-checkbox {
  position: absolute;
  top: 4px;
  right: 4px;
  z-index: 2;
