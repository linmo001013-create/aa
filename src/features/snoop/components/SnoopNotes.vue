  ...
  <div class="snoop-tab-content">
    ...
    <template v-else>
      <div
        ...
        :class="{ 'snoop-note-card--selecting': selectable }"
        @click="selectable ? $emit('toggle-select', realIndex(idx)) : toggleExpand(idx)"
      >
        <div v-if="selectable" class="snoop-note-checkbox-row">
          <div class="snoop-checkbox" :class="{ 'snoop-checkbox--checked': selectedIndices.includes(realIndex(idx)) }">
            <i v-if="selectedIndices.includes(realIndex(idx))" class="ph ph-check text-[12px]"></i>
          </div>
        </div>
...
function realIndex(sortedIdx) {
  if (!props.items) return sortedIdx
  const item = sortedItems.value[sortedIdx]
...
.snoop-note-card--selecting { cursor: pointer; }
.snoop-note-card--selecting:active { background: rgba(0, 0, 0, 0.06); }
.dark .snoop-note-card--selecting:active { background: rgba(255, 255, 255, 0.1); }

.snoop-note-checkbox-row {
  margin-bottom: 8px;
}
