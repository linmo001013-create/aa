  ...
  <div class="absolute inset-0 z-20 bg-[#F2F2F7] dark:bg-black flex flex-col">
    <div class="pt-app-lg pb-2 px-4 flex items-end justify-between">
      ...
      <div class="flex items-center gap-3">
        <button
          v-if="!store.lorebook.currentBookId"
          class="text-[#007AFF] font-semibold text-[17px]"
    ...
    <!-- Selection bar -->
    <div v-if="selectMode && selectedBookIds.length > 0" class="px-4 py-2 flex items-center justify-between bg-[#007AFF]/10 border-b border-[#007AFF]/20">
      <span class="text-[15px] text-[#007AFF] font-medium">已选择 {{ selectedBookIds.length }} 本</span>
      <button class="px-4 py-1.5 bg-red-500 text-white text-[14px] font-semibold rounded-lg active:scale-95 transition-transform" @click="deleteSelected">
        删除
      </button>
    </div>
    ...
    <div v-if="!store.lorebook.currentBookId" class="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
      ...
      <div class="space-y-2">
        ...
        <div
          ...
          :class="{ 'pl-3': selectMode }"
          @click="selectMode ? toggleBookSelect(book.id) : showEntryList(book.id)"
        >
          <div class="flex items-center gap-3 flex-1 min-w-0">
            <!-- Selection checkbox -->
            <div v-if="selectMode" class="flex-shrink-0" @click.stop="toggleBookSelect(book.id)">
              <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all"
                :class="selectedBookIds.includes(book.id) ? 'bg-[#007AFF] border-[#007AFF]' : 'border-[#C7C7CC]'">
                <i v-if="selectedBookIds.includes(book.id)" class="ph ph-check text-white text-[12px]"></i>
              </div>
            </div>
...
function toggleBookSelect(bookId) {
  const idx = selectedBookIds.value.indexOf(bookId)
  if (idx >= 0) {
...
async function deleteSelected() {
  ...
  const confirmed = await showConfirm({ message: `确定删除选中的 ${count} 本世界书？所有条目也会被删除。`, destructive: true })
  if (!confirmed) return
  store.lorebook.books = store.lorebook.books.filter(b => !selectedBookIds.value.includes(b.id))
  selectedBookIds.value = []
  selectMode.value = false
  scheduleSave()
  showToast(`已删除 ${count} 本世界书`)
}
